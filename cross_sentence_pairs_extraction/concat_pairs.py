...
inF = open("output/tacred_train_chatGPT_template_instance_dict_3.json", "r")
template_instance_dict = json.load(inF)
inF.close()

inF_all = open("output/tacred_train_chatGPT_template_instance_dict.json", "r")
template_instance_dict_all = json.load(inF_all)
inF_all.close()

inF2 = open('output/tacred_train_chatGPT_entail_edge_weight_matrix_3.txt', 'r')
...
ori_pairs = json.load(ori_pair_F)
ori_pair_F.close()


pairs = []

# generate same-template positive pairs
...
# generate mutually entailed pairs
prob_threshold = 0.95
...
pairs_01 = []
for p in ori_pairs:
    pairs_01.append([p[0][0], p[0][1]])
for r in range(0, k*num_ori-len(ori_pairs)):
    pairs_01.append([pairs[r][0], pairs[r][1]])
...
json.dump(formatted_out, open('output/tacred_train_pairs_3_095.json', 'w'))
