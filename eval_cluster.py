...
parser.add_argument('--num-cluster', default='4,6,10,16', type=str,  # 10,16,100  800,1000,1200  25000,50000,100000  10,13,16,20,50,80,100,250,500,1000  ,200,400,600,800,1000,1500,2000,3000,1000
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # validate
    sentence_train = json.load(open('data_sample_for_exemple/' + DATASET + 'tacred_test_sentence.json', 'r'))
    sentence_train_label = json.load(open('data_sample_for_exemple/' + DATASET + 'tacred_test_label_id.json', 'r'))
    ...
    #cur_dir = os.listdir()
    cur_dir = ['tac_margin_075', 'tac_margin_075_1', 'tac_margin_075_2']
    for check_dir in cur_dir:
        if os.path.isdir(check_dir) and 'measurements_margin.txt' in os.listdir(check_dir):
...
def evaluate(train_loader, model, criterion, criterion_margin, optimizer, args, chpt,cluster_result=None):
    ...
    for i, batch in enumerate(train_loader):
        ...
        # compute output
        output, target, _, output_proto, target_proto = model(sen_q=batch[:6], sen_k=batch[:5] + [batch[6]],
